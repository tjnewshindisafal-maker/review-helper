...
app.post('/negative-feedback', async (req, res) => {
  try {
    const { bizId, stars, service, feedback, department } = req.body;
    const businesses = getBusinesses();
    const biz = businesses[bizId];
    ...
    await transporter.sendMail({
      ...
      html: `<div style="font-family:sans-serif;max-width:500px;padding:20px">
        ...
        <p><b>Business:</b> ${biz?.name || bizId}</p>
        <p><b>Rating:</b> ${stars}/5</p>
        ${service ? `<p><b>Service:</b> ${service}</p>` : ''}
        ${feedback ? `<p><b>Feedback:</b> ${feedback}</p>` : ''}
        <p style="color:#888;font-size:12px">This was NOT posted on Google.</p>
      </div>`
    });
    res.json({ ok: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
